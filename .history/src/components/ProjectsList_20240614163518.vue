<template>
    <v-card color="basil">
        <v-card-title class="text-center justify-center py-6">
            <h2 class="font-weight-bold text-h2 text-basil">
                {{ qaProjectData[0].dev }}
            </h2>
        </v-card-title>

        <v-tabs v-model="selectedTab" bg-color="transparent" color="basil" grow>
        ...
        <v-tabs-items v-model="selectedTab">
            <v-tabs-item
                ...
                :key="item.text"
            >
                <v-card color="000" b-color="#4a4a4a" flat>
                    <v-card-text>
        ...
        <v-card-text v-for="item in qaProjectData[0].projects" :key="item.text">
            <v-tabs-window v-model="selectedTab">
                <v-tabs-window-item value="{{item.text}}">
                    {{ item.text }}
                </v-tabs-window-item>
            </v-tabs-window>
        </v-card-text>
