...
const f15 = () => {
    ...
    // res = (res / (a15.length || 1)).toFixed(3)
    res = res / (a15.length || 1)
...
const f17 = () => {
    ...
    a17 = Object.values(a17);
    let a17_res = a17.reduce((accum, item) => {
        accum.push(item);
        out += item + ' ';
        return accum;
...
const f20 = () => {
    let res = a20.reduce((accum, item, index) => {
        accum.unshift(item);
        return accum;
    }, [])
    console.log(res);
