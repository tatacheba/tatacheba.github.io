...
function t5() {
    ...
    let numCheck1 = +document.querySelector('.s51').value;
    let signCheck = document.querySelector('.s52').value;
    let numCheck2 = +document.querySelector('.s53').value;
    ...
    return result(numCheck1, numCheck2, signCheck)
...
document.querySelector('.b-7').addEventListener('click', function () {
    document.querySelector('.out-7').textContent = t7(65);
})
...
function t11() {
    return Array.from(arguments).reduce((accum, elem) => accum += elem);
}
...
function t12(...arg) {
    return Array.from(arg).reduce((accum, elem) => accum += elem);
}
...
function t25(arr) {
    return arr.every(elem => typeof elem == 'number' && !Number.isNaN(elem))
...
function t26() {
    ...
    if (inp.trim().length == 0) {
        return;
    } else document.querySelector('.out-26').innerHTML = inp.trim().toLowerCase();
