...
function t8() {
    let out = '',
        inp1 = +document.querySelector('.i-81').value,
...
function t12() {
    ...
    let out = '';
    while (k < div.length) {
        out += div[k].style.background = 'orange';
        k++;
    }
...
function t14() {
    let inp = document.querySelectorAll('input.i-14');
    let out = '';
    let k = 0;
    while (k < inp.length) {
        out += inp[k].checked ? inp[k].value : '';
        k++;
    }
