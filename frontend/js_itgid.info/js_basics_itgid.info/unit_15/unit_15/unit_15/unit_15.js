...
const f6 = () => {
    document.querySelector('.out-6').innerHTML = new Set(a6).size;
...
const f7 = () => {
    ...
    let out = "";
    if (inp.length == inpSet.size && inp.length > 6) {
        out = "1";
    } else {
        out = '0'
    }
...
const f9 = our_set => {
    let out = '';
    let a = Array.from(new Set(our_set));
...
const f10 = (out_set, elem) => {
    let out = "";
    let a = Array.from(new Set(out_set));
    for (i = 0; i < a.length; i++) {
        out += a[i] + " ";
    }
...
const f12 = () => {
    return Array.from(new Set(str12));
}
...
const f13 = () => {
    let str13Set = Array.from(new Set(str13));
