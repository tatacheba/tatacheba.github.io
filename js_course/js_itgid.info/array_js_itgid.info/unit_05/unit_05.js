...
let a11 = "The-Stealth-Warrior";
out = document.querySelector('.out-11');

const f11 = (str) => {
    str = a11;
    let res, res2;
    res = str.split(/[^0-9a-zA-Z]/);
    console.log(res);
    res2 = res.map((elem, i) => {
        if (i == 0) {
            return elem;
        } else {
            return callbackUpperChar(elem)
        }
    })
    console.log(res2)

    function callbackEmptyArray(elem) {
        return "An empty string was provided but not returned";
    }

    function callbackUpperChar(elem) {
        return elem.charAt(0).toUpperCase() + elem.slice(1);
    }
    out.innerHTML = res2.join("");
}
