...
const t12 = (...arg) => arg.reduce((accum, elem) => (accum > elem ? accum : elem))
...
const t13 = (max, min) => Math.floor(Math.random() * (max - min + 1)) + min;
...
const t14 = (arg) => {
    if (arg.length > arg.trim().length && arg.trim().length == 0) {
        return false;
    }
    return arg.trim();
}
...
const t15 = (num) => isNaN(num) ? false : true;
...
const t17 = (arr) => arr.every(elem => typeof elem == 'number' && !Number.isNaN(elem))
