...
function t3(event) {
    document.querySelector('.out-3').innerHTML = (event.keyCode >= 48 && event.keyCode <= 57) ? false : true;
...
function t4(event) {
    console.log(event);
    document.querySelector('.out-4').innerHTML += event.keyCode <= 90 && event.keyCode >= 65 ? '' : event.key;
}
...
function t6(event) {
    ...
    str += event.keyCode <= 90 && event.keyCode >= 65 ? '' : event.key;
    inp.value = str;
...
function t8(event) {
    ...
    document.querySelector('.out-8').textContent = a8.hasOwnProperty(event.key) ? a8[event.key] : event.key;
}
...
function t9(event) {
    // console.log(event);
    if (event.key == 'ArrowDown') valueArrowDown++;
    document.querySelector('.out-9').innerHTML = valueArrowDown;
}
...
function t11(event) {
    ...
    keyboard.forEach(item => {
        ...
        if (event.code.toLowerCase() == item.getAttribute('data-key')) {
