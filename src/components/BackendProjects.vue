<template>
  ...
  >
    ...
    <v-tabs v-model="selectedTab" height="60" slider-color="red" center-active>
      <v-tab v-for="item in projectData[0].projects" :key="item.title">
        {{ item.title }}
      </v-tab>
    </v-tabs>
      ...
      <v-tabs-window v-model="selectedTab">
        ...
        >
          <v-card class="mx-auto" color="#333" :text="item.text">
            <a href="https://nanny-oggs-cookbook.vercel.app/" target="_blank">
              <v-btn class="custom-btn" variant="outlined" block>
                {{ item.title }}
              </v-btn>
            </a>
          </v-card>
